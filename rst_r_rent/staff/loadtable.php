...
while($rtbl=mysqli_fetch_row($sel_tbl))
{
    ...
    while($rch=mysqli_fetch_row($sel_ch))
    {
        ...
        {
        ?>
<span id="a<?php echo $rch[0] ?>"><span class="glyphicon glyphicon-user" style="color: green; margin: 5px 20px;" onclick="bookchair('<?php echo $rch[0] ?>','<?php echo $o ?>','<?php echo $rtbl[0] ?>')"></span></span>
<?php
        }
        else
        {
            ?>
<span id="a<?php echo $rch[0] ?>"><span class="glyphicon glyphicon-user" style="color: red; margin: 5px 20px;" onclick="bookchair1('<?php echo $rch[0] ?>','<?php echo $o ?>','<?php echo $rtbl[0] ?>')"></span></span>
<?php
        }
        if($x==$q/2){
            ?>
<div style="width: 180px; height: 100px; background-color: black;">
